...
install_xray(){
    ...
    # 端口配置：Reality 强烈建议使用 443
    read -p "设置 xray 端口 [1-65535]（回车默认 443，强烈建议使用 443）：" port
    [[ -z $port ]] && port=443
    ...
    until [[ -z $(ss -ntlp | awk '{print $4}' | sed 's/.*://g' | grep -w "$port") ]]; do
        if [[ -n $(ss -ntlp | awk '{print $4}' | sed 's/.*://g' | grep -w "$port") ]]; then
            ...
            read -p "设置 xray 端口 [1-65535]（回车默认 443）：" port
            [[ -z $port ]] && port=443
        fi
    done
    ...
    # 目标网站设置
    read -rp "请输入配置回落的域名 [默认 www.sage.com]: " dest_server
    [[ -z $dest_server ]] && dest_server="www.sage.com"

    # Reality short-id
    short_id=$(openssl rand -hex 8)

    # Reality 公私钥 (修正解析逻辑)
    # xray x25519 输出格式已变更为：
    # PrivateKey: ...
    # Password: ... (即 PublicKey)
    keys=$(xray x25519)
...
changedest(){
    # 提取旧的 serverName
    old_dest=$(cat /usr/local/etc/xray/config.json | grep serverNames -A 1 | sed -n 2p | sed 's/\"//g' | sed 's/,//g' | sed 's/ //g')
